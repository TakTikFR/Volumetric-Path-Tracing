...
int main() {
  ...
  DielectricTexture* metal_cyan = new DielectricTexture(RGB(255, 255, 255), 1.50);
  UniformeTexture* uniforme_purple = new UniformeTexture(RGB(255, 255, 0));
  ...
  Sphere *sphere_purple = new Sphere(Point3(0.0, 1.2, -1.0), 0.5, uniforme_purple);
  Sphere *big_sphere = new Sphere(Point3(0.0, -100.5, -1.0), 100, uniforme_big);
  ...
  Sphere *sphere_cyan = new Sphere(Point3(0.0, 0.0, -3.0), 0.5, uniforme_purple);
  Sphere *big_sphere = new Sphere(Point3(0.0, -100.5, -1.0), 100, uniforme_big);
  ...
  const Vector3 pixel00 = viewportUpperLeft + (pixelDeltaU + pixelDeltaV) * 0.5;
  */
  ...
  scene.addObject(new Sphere(Point3( 0.0,  1.0, -2.5), 1.0,
  ...
  scene.addObject(new Sphere(Point3( 0.0,  0.2, -4.3), 0.2,
  ...
  Camera camera(
    ...
    Point3(0.0, 0.5, -3.0),
    Vector3(0.0, 1.0, 0.0),
    60.0, 60.0 / aspectRatio, 1.0
  );

  constexpr double focalLength    = 1.0;
  constexpr double viewportHeight = 2.0;
  constexpr double viewportWidth  = viewportHeight * (static_cast<double>(width) / height);
  ...
  for (int j = 0; j < height; ++j) {
    for (int i = 0; i < width; ++i) {
      ...
      for (int s = 0; s < SAMPLES; ++s) {
        ...
        const Vector3 pixelPos = pixel00 + pixelDeltaU * u + pixelDeltaV * v;
